...
def danger():
    ...
    # 检查 danger_list 是否是可迭代对象
    if isinstance(danger_list, int):
        danger_list = [danger_list]  # 如果是整数，转换为列表

    # 确保 danger_list 是可迭代的
    if not isinstance(danger_list, (list, tuple, set)):
        print("danger_list 不是可迭代对象，无法处理！")
        return
